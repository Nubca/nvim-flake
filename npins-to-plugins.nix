...
func
  (
    ...
    // {
      ...
      inherit version;
      vimPlugin = true;
      outPath = (if builtins ? currentSystem then mayOverride name path else path).overrideAttrs {
        pname = name;
        name = "${name}-${version}";
        inherit version;
      };
  ...
  (
    ...
    assert lib.assertMsg
      (builtins.elem json.version [
        7
        8
      ])
      ''
        Your npins lock file version does not match this local npins-to-plugins shim.
        Supported versions are 7 and 8.
      '';
    json.pins
  )
